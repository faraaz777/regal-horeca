...
async function getStockByLocation(productId) {
  ...
  const [stockRows, legacyRows] = await Promise.all([
    Stock.find({ productId: pid, qty: { $gt: 0 } })
      .select('locationId statusBucket qty')
      .lean(),
    InventoryStock.find({
      productId: pid,
      $or: [{ sellableQty: { $gt: 0 } }, { holdQty: { $gt: 0 } }],
    })
      .select('locationId sellableQty holdQty')
      .lean(),
  ]);
  ...
  for (const row of stockRows) {
    ...
    if (!byLocation.has(lid)) {
      byLocation.set(lid, { sellableQty: 0, holdQty: 0 });
    }
    const entry = byLocation.get(lid);
    if (row.statusBucket === 'sellable') entry.sellableQty += row.qty;
    else if (row.statusBucket === 'hold') entry.holdQty += row.qty;
  }

  for (const row of legacyRows) {
    const lid = row.locationId ? String(row.locationId) : '__unassigned__';
    if (!byLocation.has(lid)) {
      byLocation.set(lid, { sellableQty: 0, holdQty: 0 });
    }
    const entry = byLocation.get(lid);
    entry.sellableQty += row.sellableQty || 0;
    entry.holdQty += row.holdQty || 0;
  }

  if (byLocation.size === 0) return [];

  const locationIds = [...byLocation.keys()].filter((id) => id !== '__unassigned__');
  const locations =
  ...
  return [...byLocation.entries()]
    ...
    .filter((row) => row.sellableQty > 0 || row.holdQty > 0)
