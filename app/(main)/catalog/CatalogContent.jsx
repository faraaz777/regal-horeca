 ...
 * 
 * Handles all interactive parts of the catalog page:
 * - Filter state management
 * - URL parameter updates (shallow routing)
...
export default function CatalogContent({ 
  ...
  // Use shallow routing - update URL without navigation
  const updateURL = useCallback((updates) => {
    ...
    if (!updates.page && (updates.priceMin !== undefined || updates.priceMax !== undefined || 
        updates.colors !== undefined || updates.brands !== undefined || 
        updates.filters !== undefined || updates.sortBy !== undefined)) {
      params.delete('page');
    }

    // Use router.replace for shallow routing (no full page reload)
    router.replace(`${pathname}?${params.toString()}`, { scroll: false });
  }, [searchParams, router, pathname]);

  // Filter handlers - use shallow routing
  const handlePriceMinChange = useCallback((value) => {
    updateURL({ priceMin: value || null });
  }, [updateURL]);

  const handlePriceMaxChange = useCallback((value) => {
    updateURL({ priceMax: value || null });
  }, [updateURL]);
  ...
  const handleFilterToggle = useCallback((filterKey, value) => {
    ...
    updateURL({ filters: Object.keys(newFilters).length > 0 ? newFilters : null });
  }, [selectedFilters, updateURL]);

  const handleSortChange = useCallback((newSort) => {
    updateURL({ sortBy: newSort !== 'newest' ? newSort : null });
  }, [updateURL]);
  ...
  const clearAllFilters = useCallback(() => {
    ...
    router.replace(`${pathname}?${params.toString()}`, { scroll: false });
  }, [selectedCategorySlug, selectedBusinessSlug, searchQuery, router, pathname]);
  ...
  // Use facets from props (server-fetched)
  ...
  const totalProducts = initialPagination.total || 0;
