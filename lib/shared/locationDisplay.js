...
/**
 * Branch › Floor › Rack using codes only (warehouse labels, not display names).
 * @param {string} locationId
 * @param {Map<string, object>} byId
 */
export function formatBranchFloorRackPathCodes(locationId, byId) {
  if (!locationId || !byId?.size) return '—';
  ...
  const parts = [branch, floor, rack].map(codeLabelFor).filter(Boolean);
  if (parts.length) return parts.join(LOCATION_PATH_SEP);
