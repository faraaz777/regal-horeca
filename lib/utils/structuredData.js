...
export function generateOrganizationSchema() {
  return {
    ...
    address: {
      ...
      ...(REGAL_NAP.address.postalCode && { postalCode: REGAL_NAP.address.postalCode }),
...
/**
 * Maps storefront product fields to schema.org Offer availability.
 *
 * `productStatus` (active/inactive) is ERP lifecycle — inactive listings are delisted.
 * `status` (In Stock / Out of Stock / Pre-Order) is internal sales signaling.
 *
 * REGAL is inquiry-based B2B: catalog products are fulfillable on request, so active
 * listings default to InStock even when internal status says "Out of Stock".
 */
