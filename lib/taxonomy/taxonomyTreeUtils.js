...
/**
 * True when this visible row is the last sibling under the same parent.
 * Roots are never last-in-group — they have no parent indent to close.
 */
export function isLastDirectChildInVisibleRows(visibleRows, index) {
  const row = visibleRows[index];
  if (!row?.parentId) return false;

  for (let j = index + 1; j < visibleRows.length; j++) {
    if (visibleRows[j].parentId === row.parentId) return false;
...
/**
 * Map of row id → whether tree-guide lines should stop after this row.
 * @param {Array} visibleRows
 * @returns {Map<string, boolean>}
 */
export function getLastInSiblingGroupMap(visibleRows) {
  const map = new Map();
  for (let i = 0; i < visibleRows.length; i++) {
    map.set(visibleRows[i].id, isLastDirectChildInVisibleRows(visibleRows, i));
  }
  return map;
}

/**
 * Keep matched nodes plus ancestors (context) and descendants (nested results).
 * @param {Array} tree
 * @param {Set<string>|null} matchedIds
 */
export function filterTaxonomyTreeByMatchedIds(tree, matchedIds) {
...
export function injectSameLevelAddSlots(visibleRows, expandedIds, canAddChild, getChildLevel) {
  ...
  for (let i = 0; i < visibleRows.length; i++) {
    ...
    if (!isLastDirectChildInVisibleRows(visibleRows, i)) continue;
