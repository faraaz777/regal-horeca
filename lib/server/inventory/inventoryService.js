...
/**
 * Availability is derived from sellable qty only — dead stock does not count.
 * Returns: in_stock | low | out  (labels: IN_STOCK / LOW_STOCK / OUT_OF_STOCK)
 */
...
/**
 * Condition is derived from dead stock qty only.
 * Returns: HAS_DEAD_STOCK | NORMAL
 */
export function deriveInventoryCondition(deadStockQty) {
  return deadStockQty > 0 ? 'HAS_DEAD_STOCK' : 'NORMAL';
}
...
export async function getProductStockSummary(productId) {
  ...
  return {
    ...
    holdQty: 0,
    damagedQty: 0,
    condition: deriveInventoryCondition(totals.deadStockQty),
    lowStockThreshold: threshold,
    primaryLocation: primary?.locationId || null,
    locations: totals.rows.filter((r) => r.statusBucket !== 'sold'),
...
function aggregateStockByLocation(rows, locationById) {
  ...
  for (const row of rows) {
    ...
    if (row.statusBucket === 'dead_stock') {
      agg.deadStockQty += qty;
    } else {
      // sellable (and any legacy unknown → treat as sellable after migration)
      agg.sellableQty += qty;
    }
  }
...
export async function getProductStockDetail(productId) {
  ...
  return {
    ...
    // On-hand locations only (exclude sold history projection rows)
    locations: locations.filter((r) => r.statusBucket !== 'sold'),
...
export async function recordStockMovement({
  ...
  /**
   * Add: sellable or dead_stock only.
   * Minus/sale: always deduct from sellable (never dead stock / sold).
   */
  let effectiveBucket = statusBucket || 'sellable';
  if (action === 'add') {
    if (!INTAKE_STATUS_BUCKETS.includes(effectiveBucket)) {
      throw new Error('Add stock must be classified as sellable or dead stock');
    }
  } else if (action === 'minus') {
    effectiveBucket = 'sellable';
  } else if (!STATUS_BUCKETS.includes(effectiveBucket)) {
  ...
  if (action === 'minus') {
    ...
    const totals = await getStockTotalsForProduct(productId);
    const available = bucketQtyAtLocation(totals.rows, locId, 'sellable');
    if (qty > available) {
      const enriched = await enrichStockRowsWithLocationPaths(totals.rows);
      const row = enriched.find(
        (r) => String(r.locationId) === String(locId) && r.statusBucket === 'sellable'
      );
      throw new Error(
        `Cannot remove ${qty} — only ${available} Sellable available at ${row?.locationPath || 'selected location'}`
      );
    }
  }
...
function mapProductsToInventoryItems(products, stockByProduct, ruleByProduct, locationById, defaultThreshold) {
  return products.map((product) => {
    ...
    const condition = deriveInventoryCondition(deadStockQty);
