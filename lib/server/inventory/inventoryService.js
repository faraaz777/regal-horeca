...
import { evaluateDeadStockForProduct } from '@/lib/server/inventory/deadStockAutomationService';
...
/**
 * Attach display labels and the branch/floor/rack cascade to stock rows.
 *
 * Exported because `listAllLocations` is uncached — callers with rows for
 * several products must enrich them in a single call rather than once per
 * product.
 */
export async function enrichStockRowsWithLocationPaths(rows) {
...
export async function getProductInventoryRule(productId) {
  ...
  return {
    ...
    deadStockMarkedAt: rule.deadStockMarkedAt || null,
    deadStockArmedAt: rule.deadStockArmedAt || null,
...
export async function updateProductInventoryRule({
  ...
  const thresholdsChanged =
    String(rule.deadStockPeriod) !== String(payload.deadStockPeriod) ||
    Number(rule.deadStockQty) !== Number(payload.deadStockQty);

  const nextMarked = Boolean(payload.deadStockMarked);
  const wasMarked = Boolean(rule.deadStockMarked);
  ...
  rule.deadStockMarked = nextMarked;

  if (thresholdsChanged) {
    // Fresh grace window so edited targets are not judged immediately.
    rule.deadStockArmedAt = new Date();
  }

  if (nextMarked && !wasMarked) {
    rule.deadStockManualClear = false;
    rule.deadStockMarkedAt = new Date();
  } else if (!nextMarked && wasMarked) {
    // Sticky clear — automation must not re-mark until velocity recovers.
    rule.deadStockManualClear = true;
    rule.deadStockMarkedAt = null;
  } else if (nextMarked) {
    rule.deadStockManualClear = false;
    if (!rule.deadStockMarkedAt) rule.deadStockMarkedAt = new Date();
  }
  ...
  if (before.deadStockMarked !== after.deadStockMarked) {
    await logDeadStockTagChange({
      ...
      reason: after.deadStockMarked
        ...
        : 'Manually cleared dead stock tag (automation will not re-mark until sales recover)',
      userId,
      actorRole,
      request,
      metadata: {
        ...
        deadStockManualClear: after.deadStockManualClear,
      },
    });
  }
...
export async function getProductStockDetail(productId) {
  const product = await Product.findById(productId)
    .select('title sku stockUnit slug heroImage gallery parentProductId')
...
export async function recordStockMovement({
      ...
      await appendLedgerEntry(
        {
          ...
          ratePaise: null,
          performedBy: userId,
          soldForUserId: soldFor?.userId || null,
  ...
  if (isSale) {
    try {
      await evaluateDeadStockForProduct(productId, {
        userId,
        actorRole,
        request,
      });
    } catch (err) {
      console.error('Dead stock evaluate after sale failed:', err?.message || err);
    }
  }
...
export async function recordBatchStockMinus({
        ...
        await appendLedgerEntry(
          {
            ...
            performedBy: userId,
            /**
             * Only the sold row carries the attribution. The paired sellable
             * deduction above is the stock leaving the rack, and stamping both
             * would double-count anything grouped by this field.
             */
            soldForUserId: soldFor?.userId || null,
            soldForName: soldFor?.name || '',
          },
  ...
  if (isSale) {
    try {
      ...
      console.error('Dead stock evaluate after batch sale failed:', err?.message || err);
    }
  }
