...
const ProductSchema = new mongoose.Schema({
  ...
  variationAttributes: {
    ...
    colorSwatch: { type: String, default: '' },
    colorDetails: {
  ...
  /** Unit shown on inventory rows (Pcs, Set, kg, etc.). */
  stockUnit: {
    type: String,
    trim: true,
    default: 'Pcs',
  },
  /** Inventory master colour (standalone inventory products). */
  colour: {
    type: String,
    trim: true,
    default: '',
  ...
  /**
   * Leftover from the old inventory-add shortcut.
   * Sales reads Product.discountPercent first, then this field.
   */
  maxDiscountPercent: {
    type: Number,
    min: 0,
  ...
  /** ERP lifecycle — separate from storefront availability `status`. */
  productStatus: {
    type: String,
  ...
  /** All monetary fields on inventory SKUs are stored in paise. */
  moneyInPaise: {
    type: Boolean,
    default: false,
  },
  /**
   * MAX DISCOUNT % on the product form (variant full list).
   * Cap from MRP: lowest offer = MRP × (1 − this %). Extra room under
   * selling exists only when this is higher than the MRP→selling markdown.
   */
  discountPercent: {
    type: Number,
    min: 0,
  ...
  /**
   * Owner-only figure. Sales negotiation does not use this field.
   */
  marginPrice: {
    type: Number,
    min: 0,
