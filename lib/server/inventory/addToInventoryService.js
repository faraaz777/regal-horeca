...
export async function searchInventoryProducts(query, limit = 10) {
  ...
  return Product.find({
    ...
    .select(
      'title slug sku barcode brand colour stockUnit productStatus costPrice mrp sellingPrice price moneyInPaise heroImage categoryId departmentId'
    )
...
export async function createProductWithOpeningStock({
  ...
  return runInTransaction(async (mongoSession) => {
      ...
      [
        {
          ...
          deadStockDays: opening.deadStockDays,
    ...
    const ledgerEntry = {
      ...
      type: 'opening',
      statusBucket: opening.openingStatusBucket,
      ...
      performedBy: authSession.userId,
    };
