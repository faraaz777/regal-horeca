...
async function enrichStockRowsWithLocationPaths(rows) {
  ...
  const locs = await Location.find({ _id: { $in: ids } }).select('path code').lean();
  const pathMap = new Map(locs.map((l) => [String(l._id), l.path || l.code || '—']));
  return rows.map((r) => ({
    ...r,
    locationPath: pathMap.get(String(r.locationId)) || '—',
  }));
}
...
export async function getProductStockDetail(productId) {
  ...
  const locations = await enrichStockRowsWithLocationPaths(totals.rows);
...
export async function recordStockMovement({
  ...
  const locId =
    locationId ||
    fromLocationId ||
    (await getDefaultLocationIdForProduct(productId));
  if (!locId) {
    throw new Error('No stock location found for this product');
  }
...
export async function transferStock({
  ...
  const fromLocId = fromLocationId || (await getDefaultLocationIdForProduct(productId));
  if (!fromLocId) throw new Error('Source location not found');
