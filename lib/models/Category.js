...
// Uses .lean() for better performance and to avoid Mongoose document overhead
CategorySchema.statics.buildTree = async function(parentId = null) {
  const categories = await this.find({ parent: parentId })
    .sort({ name: 1 })
  ...
  for (const category of categories) {
    ...
    tree.push({
      ...category,
      // Ensure _id is a string (lean() already converts it, but ensure it)
      _id: category._id?.toString ? category._id.toString() : String(category._id || ''),
      // Convert parent to string or null
      parent: category.parent ? (category.parent.toString ? category.parent.toString() : String(category.parent)) : null,
