...
import {
  CATALOG_PREDEFINED_COLORS,
  canonicalizeCatalogColorName,
} from '@/lib/shared/catalogColors';

/** Map normalized color string to canonical predefined name (aggregation). */
function buildCanonicalColorSwitch(valueExpr) {
  return {
    $switch: {
      branches: CATALOG_PREDEFINED_COLORS.map((canonical) => ({
        case: { $eq: [{ $toLower: valueExpr }, canonical.toLowerCase()] },
        then: canonical,
      })),
      default: null,
    },
...
function buildBaseMatch({
  ...
  const query = {};
  // Storefront facets must exclude parent carriers; otherwise filter counts include
  // hidden parent rows that the storefront never lists, inflating sidebar numbers.
  // Child rows only count when they can appear as their own catalog cards.
  // Admin manage-products facets: all non-deleted rows (parents + hidden variants).
...
const cachedQueryProductFacets = unstable_cache(
  async ({ categorySlug, businessSlug, searchQuery, featured, status, adminMode = false } = {}) => {
    ...
    { $addFields: { _filtersArr: filtersNormalizedExpr } },
    {
      $facet: {
        colors: [
                ...
                $concatArrays: [
                  {
                    $map: {
                      ...
                      as: 'cv',
                      in: normalizeTitleCaseExpr('$$cv.colorName'),
                    },
                  },
                  {
                    $cond: [
                      ...
                      [normalizeTitleCaseExpr('$variationAttributes.color')],
                      [],
                    ],
                  },
                ],
          ...
          { $unwind: { path: '$colorNames', preserveNullAndEmptyArrays: false } },
          {
            $addFields: {
              canonical: buildCanonicalColorSwitch('$colorNames'),
            },
          },
  ...
  const colors = (res?.colors || [])
    .map((x) => canonicalizeCatalogColorName(x._id) || x._id)
    .filter(Boolean);
