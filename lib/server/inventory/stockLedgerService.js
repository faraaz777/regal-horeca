...
export async function getStockTotalsForProduct(productId) {
  ...
  // Physical on-hand excludes sold (sold is history only).
  const onHandRows = rows.filter((r) => r.statusBucket !== 'sold');
  const primary =
 ...
 * totalQty = sellable + dead_stock (sold excluded — not on hand).
 */
export async function getStockTotalsMapForProducts(productIds) {
  ...
  for (const row of rows) {
    ...
    if (row.statusBucket === 'sellable') {
      agg.sellableQty += qty;
      agg.totalQty += qty;
    } else if (row.statusBucket === 'dead_stock') {
