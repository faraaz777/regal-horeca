...
function aggregateBucketQty(stockRows) {
  const qty = { sellable: 0, hold: 0, damage: 0, scrap: 0, sample: 0 };
  for (const row of stockRows || []) {
    const q = row.qty || 0;
    if (row.statusBucket === 'sellable') qty.sellable += q;
    else if (row.statusBucket === 'hold') qty.hold += q;
    else if (row.statusBucket === 'scrap') qty.scrap += q;
    else if (row.statusBucket === 'display') qty.sample += q;
    else if (row.statusBucket === 'non_sellable') qty.damage += q;
    else qty.sellable += q;
  }
  return qty;
}
...
export function buildZoneSummariesDetailed(zones, racks, stockByRack = null) {
  return (zones || []).map((z) => {
    ...
    const stockStatusQty = { sellable: 0, hold: 0, damage: 0, scrap: 0, sample: 0 };
    ...
    for (const r of inZone) {
      ...
      else if (st === 'hold') rackStatusCounts.hold += 1;
      else if (st === 'scrap') rackStatusCounts.scrap += 1;

      stockStatusQty.sellable += r.sellableQty || 0;
      stockStatusQty.hold += r.holdQty || 0;
      stockStatusQty.scrap += r.scrapQty || 0;
      ...
      if (stockByRack) {
        ...
        for (const row of rows) {
          ...
          const q = row.qty || 0;
          if (row.statusBucket === 'display') stockStatusQty.sample += q;
          else if (row.statusBucket === 'non_sellable') stockStatusQty.damage += q;
        }
      }
    }
