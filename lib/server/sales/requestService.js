...
export async function reviewRequest(
  ...
  if (action === 'fulfill') {
    ...
    const productIds = [
      ...new Set(
        (doc.lines || [])
          .filter((line) => Number(line.approvedQty) > 0 || Number(line.requestedQty) > 0)
          .map((line) => String(line.productId))
      ),
    ];
    for (const pid of productIds) {
      try {
        await evaluateDeadStockForProduct(pid, {
          userId: session.userId,
          actorRole: session.role || '',
          request: httpRequest,
        });
      } catch (err) {
        console.error('Dead stock evaluate after fulfill failed:', err?.message || err);
      }
    }
