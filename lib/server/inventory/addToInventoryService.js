...
function buildInventoryRulePayload(opening, productId, createdBy) {
  const marked = Boolean(opening.markAsDeadStock);
  const now = new Date();
  return {
    productId,
    ...
    deadStockMarked: marked,
    deadStockMarkedAt: marked ? now : null,
    deadStockManualClear: false,
    deadStockArmedAt: now,
    createdBy,
  };
}
...
export async function addOpeningStockToExisting({
  ...
  return runInTransaction(async (mongoSession) => {
    ...
    if (!hasLedger) {
      ...
      /**
       * Additional intake must not wipe an automation/manual tag unless the
       * caller explicitly sends markAsDeadStock (boolean).
       */
      if (typeof opening.markAsDeadStock === 'boolean') {
        previousMarked = Boolean(existingRule.deadStockMarked);
        const nextMarked = opening.markAsDeadStock;
        existingRule.deadStockMarked = nextMarked;
        if (nextMarked) {
          existingRule.deadStockManualClear = false;
          if (!existingRule.deadStockMarkedAt) existingRule.deadStockMarkedAt = new Date();
        } else {
          existingRule.deadStockManualClear = true;
          existingRule.deadStockMarkedAt = null;
        }
    ...
    const nextMarked =
      typeof opening.markAsDeadStock === 'boolean'
        ? Boolean(opening.markAsDeadStock)
        : previousMarked;
    if (typeof opening.markAsDeadStock === 'boolean' && previousMarked !== nextMarked) {
