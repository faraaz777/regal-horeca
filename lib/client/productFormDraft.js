...
/**
 * Stable snapshot of editable product-form state for dirty comparison.
 * Excludes step UI and bulk-fill scratch fields. Strips ephemeral _rowId so
 * row identity churn does not false-trigger leave warnings.
 */
export function buildProductFormDirtySnapshot({
  formData = {},
  variantRows = [],
  variantFieldSelection = {},
  variantBuilderInputs = {},
  ...
  const draft = buildProductAddDraft({
    formData,
    variantRows,
    variantFieldSelection,
    variantBuilderInputs,
  ...
  const rows = (draft.variantRows || []).map((row) => {
  ...
  return JSON.stringify({
    ...
    variantRows: rows,
    variantFieldSelection: draft.variantFieldSelection || {},
    variantBuilderInputs: draft.variantBuilderInputs || {},
    hasVariantsChoice: draft.hasVariantsChoice,
  });
}

export const PRODUCT_FORM_LEAVE_MESSAGE =
  'You have unsaved changes. Leave this page? Your edits will not be saved.';
