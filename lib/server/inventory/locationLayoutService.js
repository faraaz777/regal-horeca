...
/**
 * sellableQty at a rack = physical pieces at that location.
 */
...
/** Rack colour / status from sellable availability. Dead stock is a product TAG. */
export function deriveRackStockStatus({ sellableQty }, threshold) {
  return deriveStockStatus(sellableQty, threshold);
}
...
function buildRackDto(rack, doc, stockAgg, threshold, byId, coordinateWidth, coordinateHeight) {
...
export async function getFloorLayout(floorId) {
  ...
  const totalCapacity = allRacks.reduce((sum, r) => sum + (r.capacity || 0), 0);
  ...
  return {
    ...
    zoneSummaries: zoneSummaryList.map((s) => ({
      ...
      name: s.name,
      ...s.summary,
      lowStockRackCount: s.summary.rackStatusCounts?.low ?? 0,
      emptyRackCount: s.summary.rackStatusCounts?.empty ?? 0,
      capacity: s.summary.totalCapacity,
    })),
    racks: placed,
