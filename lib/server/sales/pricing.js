...
/**
 * List / margin / min-offer snapshot for catalog cards, detail, and quote lines.
 * negotiablePercent is derived for display only.
 */
export function getProductSalesPricing(product) {
  const listPricePaise = getListPricePaise(product);
  const marginPricePaise = toPaise(product, 'marginPrice');
  const minOfferPricePaise = getMinAllowedPricePaise(listPricePaise, marginPricePaise);
  const negotiablePercent = toDisplayNegotiablePercent(
    getNegotiablePercent(listPricePaise, minOfferPricePaise)
  );
  return { listPricePaise, marginPricePaise, minOfferPricePaise, negotiablePercent };
}
...
export async function loadProductForSales(productId) {
  const product = await Product.findOne({
    ...
    .select(
      'title slug sku barcode brand mrp sellingPrice price marginPrice moneyInPaise heroImage gallery parentProductId stockUnit productStatus'
    )
