...
function aggregateBucketQty(stockRows) {
  const qty = { sellable: 0, dead_stock: 0 };
  for (const row of stockRows || []) {
    const q = row.qty || 0;
    if (row.statusBucket === 'sellable') qty.sellable += q;
    else if (row.statusBucket === 'dead_stock') qty.dead_stock += q;
  }
  return qty;
}
...
export function buildZoneSummariesDetailed(zones, racks, stockByRack = null) {
  return (zones || []).map((z) => {
    ...
    const stockStatusQty = { sellable: 0, dead_stock: 0 };
    ...
    for (const r of inZone) {
      ...
      stockStatusQty.dead_stock += r.deadStockQty || 0;
