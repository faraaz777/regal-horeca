 ...
 * @returns {Map<string, { sellableQty: number, holdQty: number, scrapQty: number, totalQty: number }>}
 */
export async function getStockTotalsMapForProducts(productIds) {
  ...
  for (const row of rows) {
    ...
    const qty = row.qty || 0;
    agg.totalQty += qty;
    if (row.statusBucket === 'sellable') agg.sellableQty += qty;
    else if (row.statusBucket === 'hold') agg.holdQty += qty;
    else if (row.statusBucket === 'scrap') agg.scrapQty += qty;
  }
