...
/** Locator presence — empty vs has stock (not min-stock planning). */
function deriveRackStockPresence(stockAgg, productCount = 0) {
  const qty = Number(stockAgg?.sellableQty) || 0;
  return qty > 0 || productCount > 0 ? 'has_stock' : 'empty';
}

/** sellableQty at rack = physical pieces from Stock snapshot. */
...
async function loadFloorContext(floorId) {
  ...
  const [rackDocs, stocks, products] = await Promise.all([
    rackIds.length
...
function buildRackOptionItem(rackDoc, ctx, zoneId) {
  ...
  const productCount = productSet?.size ?? 0;
  const stockPresence = deriveRackStockPresence(stockAgg, productCount);
  ...
  return {
    ...
    distinctProductCount: productCount,
    productCount,
    stockPresence,
...
export async function getZoneSummary(floorId, zoneId) {
  ...
  for (const rack of ctx.cascadeRacks) {
    ...
    const stockAgg = aggregateRackStock(ctx.stockByRack.get(rid) || []);
    const productCount = ctx.productsByRack.get(rid)?.size ?? 0;
    const stockPresence = deriveRackStockPresence(stockAgg, productCount);
    rackDtos.push({
      _id: rid,
      stockPresence,
