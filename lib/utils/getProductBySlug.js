...
export const getFullProductBySlug = cache(async (slug) => {
  ...
  try {
    ...
    // Attach sibling children for PDP swatch pickers (only when this product belongs
    // to a parent group). Standalones get an empty list.
    if (product.parentProductId) {
      product.children = await getSiblingChildren(product.parentProductId);
    }
