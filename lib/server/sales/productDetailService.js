...
async function getStockByLocation(productId) {
  ...
  for (const row of stockRows) {
    ...
    if (!byLocation.has(lid)) {
      byLocation.set(lid, { sellableQty: 0, deadStockQty: 0 });
    }
    const entry = byLocation.get(lid);
    if (row.statusBucket === 'sellable') entry.sellableQty += row.qty;
    else if (row.statusBucket === 'dead_stock') entry.deadStockQty += row.qty;
  }
  ...
  return [...byLocation.entries()]
    ...
    .filter((row) => row.sellableQty > 0 || row.deadStockQty > 0)
...
function mapProductCore(resolved, productId, stockSummary) {
  ...
  return {
    ...
    deadStockQty: stockSummary.deadStockQty || 0,
