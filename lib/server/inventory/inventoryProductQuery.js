 ...
 */
export async function buildInventoryProductQuery({ search = '', categoryId = '' } = {}) {
...
/**
 * Product ids that entered inventory (opening stock / ledger) or have stock at a location.
 */
export async function getInventoryTrackedProductIds() {
  const [ledgerIds, stockIds, ruleIds, legacyIds] = await Promise.all([
    StockLedger.distinct('productId'),
    Stock.distinct('productId'),
    InventoryRule.distinct('productId'),
    InventoryStock.distinct('productId'),
  ]);

  const seen = new Set();
  for (const id of [...ledgerIds, ...stockIds, ...ruleIds, ...legacyIds]) {
 ...
 */
export async function buildInventoryLedgerProductQuery({ search = '', categoryId = '' } = {}) {
  const trackedIds = await getInventoryTrackedProductIds();
  if (trackedIds.length === 0) {
    return { _id: { $in: [] } };
  }

  const baseMatch = await buildInventoryProductQuery({ search, categoryId });
  return {
