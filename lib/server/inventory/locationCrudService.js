...
import {
  assertCanAddRackToFloor,
  seedDefaultRacksForFloor,
} from '@/lib/server/inventory/rackLimitsService';
...
export async function createLocation({ code, name, level, parentLocationId }) {
  ...
  if (level === 'rack' && parentLocationId) {
    const parent = await Location.findById(parentLocationId).lean();
    const floorId = parent?.level === 'floor' ? parent._id : null;
    if (floorId) {
      await assertCanAddRackToFloor(floorId);
    }
  }
  ...
  const doc = await Location.create({
    ...
    path: trimmedCode,
    isActive: true,
    ...(level === 'rack' ? { capacity: MAX_PRODUCTS_PER_RACK } : {}),
  });

  if (level === 'floor') {
    await seedDefaultRacksForFloor(doc._id);
  }
...
export async function updateLocation(id, { code, name, level, parentLocationId, isActive, capacity }) {
  ...
  // Only re-validate hierarchy when level/parent actually change. This avoids
  // rejecting racks that legitimately sit under legacy section/zone nodes when
  // only capacity/name/isActive is being edited.
