...
import { buildCodePath } from '@/lib/server/inventory/locationTreeService';
...
export async function validateParentLevel(level, parentLocationId) {
  ...
  const expectedParentLevel = LOCATION_LEVELS[LEVEL_INDEX[level] - 1];
...
async function rebuildStoredPath(loc, byId) {
  const displayPath = buildCodePath(loc, byId);
...
export async function createLocation({ code, name, level, parentLocationId }) {
  ...
  if (!LOCATION_LEVELS.includes(level)) throw new Error('Invalid location level');
  ...
  return {
    ...fresh,
    displayPath: buildCodePath(fresh, byId),
  };
}
...
export async function updateLocation(id, { code, name, level, parentLocationId, isActive }) {
  ...
  const byId = new Map((await Location.find({}).lean()).map((l) => [String(l._id), l]));
  return { ...fresh, displayPath: buildCodePath(fresh, byId) };
}
...
export async function getLocationById(locationId) {
  ...
  const byId = new Map((await Location.find({}).lean()).map((l) => [String(l._id), l]));
  return { ...loc, displayPath: buildCodePath(loc, byId) };
}
