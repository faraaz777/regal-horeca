...
import { releaseSlugForActiveInsert } from '@/lib/server/products/slugArchive';
...
export async function generateUniqueSlug(title, excludeProductId = null) {
  ...
  // Build base query - exclude current product if updating; only active (non-deleted) rows count for uniqueness
  ...
  if (!existingBaseProduct) {
    await releaseSlugForActiveInsert(baseSlug);
    return baseSlug;
  }
  ...
  await releaseSlugForActiveInsert(nextSlug);
  return nextSlug;
}
