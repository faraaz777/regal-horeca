...
//   if (!response.ok || !data.success) {
//     // Include both the error message and details if available
//     const errorMessage = data.error || 'Upload failed';
//     const errorDetails = data.details ? `: ${data.details}` : '';
//     throw new Error(`${errorMessage}${errorDetails}`);
//   }
...
//     // Only initialize if this is a different product or first time
//     if (product && initializedProductIdRef.current !== currentProductId) {
//       const categoryId = product.categoryId?._id || product.categoryId;
//       setFormData({
//         ...product,
...
//       // Set category ancestry (only if categories are available)
//       if (categoryId && categories.length > 0) {
//         const ancestry = getCategoryAncestry(categoryId, categories);
//         setCategorySelection(ancestry);
//       }
...
//       initializedProductIdRef.current = null;
//     }
//   }, [product]); // Only depend on product, not categories
...
//   const getCategoriesByParent = (parentId) => {
//     if (!parentId) return categories.filter(c => {
//       const cParent = c.parent?._id || c.parent;
//       return !cParent;
//     });
