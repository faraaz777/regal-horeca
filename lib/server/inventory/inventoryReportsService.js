...
/**
 * Warehouse on-hand qty and cost value without paginating every rack row.
 * Used by the owner dashboard glance.
 */
export async function getSellableStockTotals() {
  const [row] = await Stock.aggregate([
    { $match: { statusBucket: 'sellable', qty: { $gt: 0 } } },
    ...
    { $unwind: { path: '$product', preserveNullAndEmptyArrays: true } },
    {
      $project: {
        ...
        unitCost: {
          $cond: [
            { $eq: ['$product.moneyInPaise', true] },
            { $divide: [{ $ifNull: ['$product.costPrice', 0] }, 100] },
...
export async function getDeadStockReport({ search = '', page = 1, limit = 50 } = {}) {
  const taggedRules = await InventoryRule.find({ deadStockMarked: true })
    .select('productId deadStockPeriod deadStockQty deadStockMarkedAt updatedAt createdAt')
    .lean();
  ...
  let items = products.map((product) => {
    ...
    const cost = product.costPrice || 0;
    const unitCost = product.moneyInPaise ? cost / 100 : cost;
    /** Days since tagged dead — not last warehouse activity. */
    const taggedAt = rule?.deadStockMarkedAt || rule?.updatedAt || rule?.createdAt || null;
    return {
      productId: pid,
      title: product.title,
      sku: product.sku || '',
      ...
      unitCost,
      value: unitCost * qty,
      lastLedgerAt: taggedAt,
      isDeadStock: true,
      ageingDays: taggedAt
        ? Math.max(0, Math.floor((now - new Date(taggedAt).getTime()) / 86400000))
        : null,
      statusBucket: 'tagged',
    };
  });
...
/**
 * Sold movement history.
 *
 * `soldForName` is whose sale it was. `recordedByName` is who keyed it in.
 * They are different people on request-driven sales (salesman vs inventory)
 * and often different on walk-in sales too.
 */
export async function getSoldMovementReport({
  ...
  let items = entries.map((e) => {
    ...
    const recordedBy = e.performedBy?.name || e.performedBy?.email || '—';
    /**
     * Stored name wins: it was captured at the sale, so a later rename
     * does not rewrite history. Blank means this row is from before
     * Sold For existed, or a manual Sold that was never attributed.
     */
    const soldFor = e.soldForName || '—';
    return {
      _id: String(e._id),
  ...
  if (search.trim()) {
    ...
    items = items.filter(
      (i) =>
        ...
        i.soldForName.toLowerCase().includes(q) ||
