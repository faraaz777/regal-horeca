...
import { assertRackCanAcceptProduct } from '@/lib/server/inventory/rackLimitsService';
...
async function validateOpeningLocations(entries, productId = null) {
  ...
  for (const { locationId, qty } of entries) {
    ...
    await assertRackCanAcceptProduct(locationId, productId);
...
export async function createProductWithOpeningStock({
  ...
  await validateOpeningLocations(locationEntries, null);
