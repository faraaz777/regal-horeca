...
import { normalizeRackParentsToFloor } from '@/lib/server/inventory/floorRacks';
...
export async function getLocationTree() {
  /**
   * Heal rack parents to nearest floor so Locations tiles match Allocate cascade.
   * Idempotent — no-op when parents are already floors.
   */
  ...
  const items = stocks
    .map((row) => {
      ...
      return {
        ...
        sku: product.sku || '',
        qty: row.qty,
        sellableQty: row.statusBucket !== 'sold' ? row.qty : 0,
        stockUnit: product.stockUnit || 'Pcs',
        statusBucket: row.statusBucket === 'sold' ? 'sold' : 'sellable',
