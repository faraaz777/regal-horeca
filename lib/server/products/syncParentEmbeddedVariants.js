  ...
  const result = await Product.updateOne(
    ...
    },
    {
      $set: {
        ...
        'variants.$.colorHex': String(attrs.colorHex || attrs.colorDetails?.colorHex || '').trim(),
        'variants.$.colorSwatch': String(attrs.colorSwatch || attrs.colorDetails?.swatch || '').trim(),
        'variants.$.colorDetails': attrs.colorDetails || (attrs.color ? {
          colorName: String(attrs.color).trim(),
          colorHex: String(attrs.colorHex || '').trim(),
          swatch: String(attrs.colorSwatch || '').trim(),
        } : null),
