...
const EnquirySchema = new mongoose.Schema({
  ...
  assignedTo: {
    type: String,
    trim: true,
    default: '', // Legacy free-text; use assignedToUserId for new data
  },
  assignedToUserId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
...
EnquirySchema.index({ assignedToUserId: 1, status: 1 });
...
// Additional indexes for search queries (safely added - will build in background)
EnquirySchema.index({ name: 1 }); // For name regex searches
EnquirySchema.index({ phone: 1 }); // For phone regex searches
EnquirySchema.index({ message: 1 }); // For message regex searches
