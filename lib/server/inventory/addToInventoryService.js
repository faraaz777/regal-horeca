...
export async function listSelectableLocations() {
  const { listAllLocations } = await import('@/lib/server/inventory/locationCrudService');
  const { attachLocationStockCounts } = await import('@/lib/server/inventory/locationTreeService');
  const locations = await listAllLocations();
  const selectable = locations.filter((loc) => loc.level === 'rack' || loc.level === 'shelf');
  return attachLocationStockCounts(selectable);
}
...
function buildOpeningLedgerEntry(opening, productId, performedBy) {
  return {
    productId,
    locationId: opening.locationId,
    type: 'opening',
    statusBucket: opening.openingStatusBucket || 'sellable',
    qty: opening.openingQty,
...
export async function createProductWithOpeningStock({
  ...
  const location = await Location.findById(opening.locationId).lean();
  ...
  return runInTransaction(async (mongoSession) => {
    ...
    const ledgerEntry = buildOpeningLedgerEntry(opening, product._id, authSession.userId);
